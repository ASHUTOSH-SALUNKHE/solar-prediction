...
import { createWeatherData } from '../../../convex/plans';
    ...
    try {
      ...
      const monthlyDataToInsert = Object.keys(monthlyAverages).map((key) => {
        ...
        return {
          ...
          average_humidity: averages.average_humidity === null ? undefined : averages.average_humidity,
      ...
      <form onSubmit={handleSubmit} className = "w-full max-w-md ">
        <Card className="max-w-md mx-auto p-6 bg-black rounded-xl shadow-md space-y-6  ">
          <CardHeader className="space-y-2 text-center">
            <CardTitle className="text-2xl font-bold text-white">Enter Your Location</CardTitle>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-col space-y-2">
              <label htmlFor="latitude" className="text-sm font-medium text-white">
                Latitude
              </label>
              <input
                id="latitude"
            ...
            <div className="flex flex-col space-y-2">
              <label htmlFor="longitude" className="text-sm font-medium text-white">
                Longitude
              </label>
              <input
                id="longitude"
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                {locationName ? (
                  <>
                    <span className="font-semibold">Address:</span> {locationName}
                  </>
                ) : (
      ...
      for (let month = 0; month < 12; month++) {
        ...
        if (data) {
          monthlyAverages[month] = {
            min_temperature: parseFloat((data.tempMinSum / data.tempMinCount).toFixed(2)),
            max_temperature: parseFloat((data.tempMaxSum / data.tempMaxCount).toFixed(2)),
            average_humidity: humidity
              ? parseFloat((data.humiditySum / data.humidityCount).toFixed(2))
              : null,
            average_windspeed: parseFloat((data.windspeedSum / data.windspeedCount).toFixed(2)),
            average_radiation: parseFloat((data.radiationSum / data.radiationCount).toFixed(2)),
          };
        }
      }
